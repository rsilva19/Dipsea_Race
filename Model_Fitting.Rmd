...
mod_Sex <- mgcv::gam(Actual_Time ~ s(Age) + s(Age, by = as.factor(Sex), k = 20) + as.factor(Sex), data = ds) 
summary(mod_Sex) #.295
plot(mod_Sex, se = T)
...
mod_all_sep <- mgcv::gam(Actual_Time ~ s(Age, k= 60) + Sex + Section + s(Age, k= 60, by = as.factor(Sex)) + s(Age, k= 60, by = as.factor(Section)) , data = ds)  #.503
